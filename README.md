...
The template uses a local Postgres database. Application expects that PG will be available on 5432 with credentials from .env. By default it's:
...
To run local database use the following command:

```bash
DB_HOST=5432 docker-compose up db
```
